...
$productCsvData=[];
$filtedCsvData=[];
$modelProductFile=fopen(__DIR__."/modelproduct/model_product.csv","r");
$allproductFile= fopen(__DIR__."/csvupload/Product.csv","r"); 
$x=0;
while (($modelProductData = fgetcsv($modelProductFile)) !== FALSE) {
    if($x != 0) {
        ...
        if($productId == 0) {
           ...
           $modelData[]=array($parentSku);
        }
    }
     $x++;
}
fclose($modelProductFile);
while (($productData = fgetcsv($allproductFile,10000,',')) !== FALSE) {
...
// $fileName='model-Product-'.date("d-m-y").'-'.time().'.csv';
// $downloadDir= __DIR__.'/modelproduct/ModelProduct/'.$fileName;
