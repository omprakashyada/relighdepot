...
// $fileName='color-varient-'.date("d-m-y").'-'.time().'.csv';
// $downloadDir= __DIR__.'/varient-data/'.$fileName;
...
@$filtedCsvData[]=array('ProductId','Product Name','Product Code/SKU');

$fileName='ModelData-'.date("d-m-y").'-'.time().'.csv';
$downloadDir= __DIR__.'/varient-data/'.$fileName;
$fileOpen = fopen($downloadDir, "w");
foreach ($productCsvData as $csvData) {
        $productid=$csvData[1]; 
        $productName=$csvData[2];
        $productSkuCode=$csvData[4];
    @$filtedCsvData[]=array($productid,$productName,$productSkuCode);
}
    foreach($filtedCsvData as $newData){
        fputcsv($fileOpen,$newData);
    }
