                     ...
                     <ul>
                        ...
                        <li class="menu-item menu-itemsopen">
                           ...
                           <ul class="menu-item-1 collapse" id="demo2">
                              <li>
                                 <a href="https://pim.sdiebiz.net/relight/live_delete.php" class="active">
                                 <i class="fa fa-pencil-square-o" aria-hidden="true"></i> Live Site
                                 </a>
                              </li>
                              <li>
                  ...
                  <div class="block-element">
                     <div class="selct-site stage-slect">
                        <div class="site-text data-del">
                           <h3>Search Your Live Product For Delete</h3>
                        </div>
...
<?php 
 ...
 if(isset($_REQUEST['Sku'])) {
   $sku=$_POST['sku_val'];
   $query = "SELECT DISTINCT product_id,product_sku,product_name FROM  live WHERE product_sku LIKE '%$sku%'";
...
<div class="product-inner-sec product-page inner-product-table  inner-c inner-delete-btn"> 
    <div class="container">
                  ...
                  <table id='example' class='show-user-table display table-responsive' style='width:100%'>
                     ...
                     <tbody class="tbody">
                           ...
                           <tr>
                              ...
                              <td><button class="btn-danger table-del-btn"><a class="del-btn text-white" href="https://pim.sdiebiz.net/relight/live_delete.php?product_id=<?php echo $row['product_id']; ?>&str=<?php echo $sku;?>" onclick='javascript:confirmationDelete($(this));return false;' target='_blank'>Delete
                              </a></button></td> 
                           </tr>
                           <?php  
                              }}
      ...
      if(isset($_GET['str'])){
         ...
         $str=$_GET['str'];
         $query = "SELECT DISTINCT product_id,product_sku,product_name FROM live WHERE product_sku LIKE '%$str%'";
         //$query="SELECT * FROM staging WHERE product_sku='".$sku."'";
         $results = $conn->query($query);
         $num = $results->num_rows;
                  ...
                  <table id='example' class='show-user-table display table-responsive' style='width:100%'>
                    ...
                    <tbody class="tbody">
                           ...
                           <tr>
                              ...
                              <td><?php echo $row['product_sku'];?> </td>
                              <td class="product-name"><?php echo $row['product_name'];?></td> 
                              <td><button class="btn-danger"><a class="del-btn text-white" href="https://pim.sdiebiz.net/relight/live_delete.php?product_id=<?php echo $row['product_id']; ?>&str=<?php echo $str;?>" onclick='javascript:confirmationDelete($(this));return false;' target='_blank'>Delete
                              </a></button></td> 
                           </tr>
                           <?php       
...
if ($conn->connect_error) {
  ...
  $product_id = $_GET['product_id']; 
  $str=$_GET['str'];
  $del = "DELETE FROM live WHERE product_id='".$product_id."'";// delete query

if($del) {
   $results = $conn->query($del);
      if($results) {
         header('location:https://pim.sdiebiz.net/live_delete.php?product_id='.$product_id.'&str='.$str); 
      } else {
