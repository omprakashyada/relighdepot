                     ...
                     <ul>
                        <li class="menu-item menu-itemsopen">
                           ...
                           <ul class="menu-item-1 collapse" id="demo1">
                              <li>
                                 <a href="http://localhost/relight/live_selection.php">
                                 <i class="fa fa-pencil-square-o" aria-hidden="true"></i> Live Site
                                 </a>
                              </li>
                              <li>
                                 <a href="http://pim.sdiebiz.net/relight/staging_selection.php">
                  ...
                  <div class="attr-top-sec">
                   ...
                   <div class="download-pdf-entire-sec download-sale-pdf">
                     ...
                     $conn = new mysqli($servername, $username, $password, $database);
                      ...
                      //$path_new = "/home/rdevarona/pim.sdiebiz.net/public/relight/newcsv/"; 
                        $path_new =  __DIR__."/newcsv/"; 
                        $latest_ctime_new = 0;
                        $latest_filename_new = '';  
                        $d_new = dir($path_new);
                        while (false !== ($entry_new = $d_new->read())) {
                        $filepath_new = "{$path_new}/{$entry_new}";
                        //could do also other checks than just checking whether the entry is a file
                           if (is_file($filepath_new) && filectime($filepath_new) > $latest_ctime_new) {
                           $latest_ctime_new = filectime($filepath_new);
                           $latest_filename_new = $entry_new;
                           }
                        }
                        $res='';
                        $selling_price = 0; 
                        //($handle_new = fopen('/home/rdevarona/pim.sdiebiz.net/public/relight/newcsv/'."$latest_filename_new", "r")) !== FALSE
                        if (($handle_new = fopen(__DIR__.'/newcsv/'."$latest_filename_new", "r")) !== FALSE){  
                           $selling_new=array();  
                           $Newsorted[]=array();
                           $New_Price[]=array();
                        ...
                        $New_Price=array($New_Col);
                           foreach ($Newsorted as $key => $value) {
                           if($key !=0) {
                              $New_Price[]=$value;
                           }
                        }
                        ...
                        $Newfile='New-SalePrice'.'-'.time().'-'.date("d-m-y").'.csv';
                        // $directory_new="/home/rdevarona/pim.sdiebiz.net/public/relight/SortedCsvPrice/".$Newfile;
                        $directory_new=__DIR__."/SortedCsvPrice/".$Newfile;
                        $file_n = fopen($directory_new, "w");
                        $Ncount=count($New_Price)-1;
                        foreach ($New_Price as $line) {
                           fputcsv($file_n, $line);
                        }
                       ...
                       ?>
                        <div class="download-pdf-sec">
                           <div class="modify">
                             <h2 class="maps-title_exist">Total Number of New Products:<span><?php echo  $Ncount;?></span></h2> 
                              <div class="pdf-icon"> 
                                 <i class="fa fa-file-pdf-o" aria-hidden="true"></i>
                              </div>
                              <!-- <a href="https://pim.sdiebiz.net/relight/SortedCsvPrice/< ?php echo $Newfile; ?>" id="download" class="export-btn"> -->
                              <a href=" http://localhost/relight/SortedCsvPrice/<?php echo $Newfile; ?>" id="download" class="export-btn">
